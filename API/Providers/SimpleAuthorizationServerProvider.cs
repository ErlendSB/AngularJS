    ...
    public class SimpleAuthorizationServerProvider : OAuthAuthorizationServerProvider
    {
        ...
        public override async Task GrantResourceOwnerCredentials(OAuthGrantResourceOwnerCredentialsContext context)
        {
            ...
            using (AuthRepository _repo = new AuthRepository())
            {
                ...
                using (PhoneDBContext db = new PhoneDBContext())
                {
                    CompanyID = db.UserInfoes.Where(c => c.UserId == UserID).Select(s => s.CompanyID).FirstOrDefault();
                }
                var identity = new ClaimsIdentity(context.Options.AuthenticationType);
                identity.AddClaim(new Claim("companyid", CompanyID.HasValue ? CompanyID.Value.ToString() : string.Empty));
                identity.AddClaim(new Claim(ClaimTypes.Name, context.UserName));
                identity.AddClaim(new Claim("sub", context.UserName));
                foreach (var role in await _repo.GetRoles(UserID))
